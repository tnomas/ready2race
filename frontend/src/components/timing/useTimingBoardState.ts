...
import {TimeMarkDto, TimingResultDto, TimingSequenceDto, TimingStationDto} from '@api/types.gen.ts'
...
export function applyWsMessage(marks: BoardMark[], message: TimingWsMessage): BoardMark[] {
    switch (message.type) {
        ...
        case 'sequenceChanged':
            ...
            return marks
        case 'resultChanged':
            // Result-table data, not mark data — handled by the optional `onResultChanged`
...
export function useTimingBoardState(
    ...
    const markSaved = useCallback((id: string) => {
        setAllMarks(prev =>
            prev.map(m => (m.id === id ? {...m, pending: false, failed: false} : m)),
        )
    }, [])
