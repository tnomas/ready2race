...
const TeamCaptureGrid = ({
    ...
    useEffect(() => {
        setArmed(prev => {
            const next = prev.filter(
                id =>
                    !finishedTeams.has(id) && teams.some(team => team.competitionMatchTeam === id),
            )
    ...
    const unambiguousByStartNumber = useMemo(() => {
        ...
        for (const team of teams) {
            if (team.startNumber == null) continue
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            const {
                armed: armedNow,
                captureTeam: fire,
                unambiguousByStartNumber: byNumber,
            } = handlersRef.current
                    ...
                    {armed.map((teamId, index) => {
                        ...
                        return (
                            <Chip
                                ...
                                onDelete={() => setArmed(prev => prev.filter(id => id !== teamId))}
