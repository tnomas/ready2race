...
}): string => JSON.stringify([choice.competitionId, choice.ratingCategoryName ?? null])
...
export const ceremonyRequestKey = (choice: AwardCeremonyChoiceDto): AwardCeremonyKeyRequest => ({
    ...
    ratingCategoryName: choice.ratingCategoryName ?? null,
})
...
export const isSingleUncategorized = (group: CeremonyGroup): boolean =>
    group.ceremonies.length === 1 && (group.ceremonies[0].ratingCategoryName ?? null) === null
