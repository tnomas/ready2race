...
// eingebautes localStorage. Für die Tests reicht eine minimale In-Memory-Nachbildung —
// die Härtung im Modul selbst wird über die beiden Fälle unten geprüft, in denen
...
describe('myEventStorage', () => {
    ...
    it('merkt einen Code und liest ihn zurück', () => {
    ...
    it('hält mehrere Codes nebeneinander', () => {
        rememberMyEventCode({qrCode: 'abc', eventId: eventA})
        rememberMyEventCode({qrCode: 'def', eventId: eventA})
        expect(readMyEventCodes()).toHaveLength(2)
    })

    it('überschreibt denselben Code statt ihn zu doppeln', () => {
        rememberMyEventCode({qrCode: 'abc', eventId: eventA})
    ...
    it('behält den bekannten Namen, wenn derselbe Code erneut gescannt wird', () => {
    ...
    describe('Reihenfolge und Auswahl', () => {
        it('hält die Reihenfolge des ersten Scans, auch beim erneuten Scan', () => {
            rememberMyEventCode({qrCode: 'abc', eventId: eventA})
            rememberMyEventCode({qrCode: 'def', eventId: eventA})
            rememberMyEventCode({qrCode: 'abc', eventId: eventA})
        ...
        it('wählt den zuletzt gescannten Code aus', () => {
            rememberMyEventCode({qrCode: 'abc', eventId: eventA})
            rememberMyEventCode({qrCode: 'def', eventId: eventA})
            expect(activeCodeForEvent(eventA)?.qrCode).toBe('def')
        })

        it('wählt auch dann den zuletzt gescannten Code, wenn er schon in der Liste stand', () => {
        ...
        it('liefert nichts, wenn für die Veranstaltung kein Code hinterlegt ist', () => {
            rememberMyEventCode({qrCode: 'abc', eventId: eventB})
            expect(activeCodeForEvent(eventA)).toBeUndefined()
        })

        it('wählt Alteinträge ohne Zeitstempel in Listenreihenfolge', () => {
        ...
        it('trägt den Namen nach, ohne Position und Auswahl zu verändern', () => {
            rememberMyEventCode({qrCode: 'abc', eventId: eventA, lastSeenAt: 1000})
            rememberMyEventCode({qrCode: 'def', eventId: eventA, lastSeenAt: 2000})
        ...
        it('ignoriert einen Namen für einen unbekannten Code', () => {
            rememberMyEventDisplayName('abc', 'Nina Nordmann')
            expect(readMyEventCodes()).toEqual([])
        })
    })
    ...
    it('liefert eine leere Liste bei kaputtem Speicherinhalt', () => {
        // Ein von Hand verbogener oder von einer älteren Version geschriebener Eintrag
        // darf die Ergebnisseite nicht zerlegen.
        localStorage.setItem(MY_EVENT_STORAGE_KEY, '{kein json')
        expect(readMyEventCodes()).toEqual([])
    })

    it('verwirft Einträge ohne Pflichtfelder', () => {
