...
import {getServerTime} from '@api/sdk.gen.ts'
import {ClockQuality, ClockSync} from '@utils/timing/serverClock.ts'

export interface UseTimingServerClockResult {
    /**
...
/**
 * Hook for server clock synchronization of the timing surfaces.
 *
 * Deliberately separate from `components/event/info/athleteBoard/useServerClock.ts`: that one takes
 * the server instant handed to it inside a board payload it is polling anyway, while capturing a race
 * time needs its own lowest-latency sampling against `POST /timing/serverTime` (five-second cadence,
 * best-roundtrip wins) and a quality signal the capture button can disable itself on.
 *
 ...
 */
export function useTimingServerClock(): UseTimingServerClockResult {
