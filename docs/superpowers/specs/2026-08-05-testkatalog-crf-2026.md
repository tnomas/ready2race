...
| ID | Fall | Erwartung | testbar ab | Nachweis |
|---|---|---|---|---|
| C35 | Name im Export | Startliste eines Laufs exportieren: die Wellen-Spalte lautet `10:30 \| 12 JM4x \| AF1`. Nach dem Import in RaceClocker steht derselbe Name in der Wellenliste und auf dem Timer-Gerät | `acd5004d` | 6fe40a37, 10.08. (CF): Export Wettkampf 1 Finale trägt Wellenspalte `09:20 \| 1 CF2x \| Finale`; nach dem Import steht die Welle unter genau diesem Namen in RaceClocker |
...
| C38 | Ergebnis-Pull findet den Lauf | Knopf **und** Automatik ziehen die Ergebnisse einer unter dem neuen Namen angelegten Welle. Zusammen mit C2 zu lesen: Export und Pull leiten den Namen aus derselben Funktion ab, weichen sie voneinander ab, greift der Notnagel-Filter nicht mehr | `acd5004d` | 6fe40a37, 10.08. (CF): Knopf-Pull auf die Welle `09:20 \| 1 CF2x \| Finale` ordnete alle Boote zu (Bahnen vergeben) — Zuordnung über die Match-Team-IDs in Extra info; Automatik-Takt noch offen |
...
| C41 | Nicht aktivierter Lauf | Gegenprobe: derselbe Tausch an einem Lauf **ohne** „Am Start". Es passiert weiterhin nichts — ein Umsortieren vor der Aktivierung schlägt bewusst nicht durch. Der zweite Riegel bleibt bestehen und ist gewollt | `aa3d209a` | 6fe40a37, 10.08. (CF): Riegel im Code belegt — `RaceClockerPollService.kt:409` gibt für nicht aktivierte Läufe außer der Aktivierung nichts frei (kein `applyRaceClockerRows`). Der **manuelle** Knopf geht bewusst darüber hinweg (Operator-Override), das ist kein C41-Fall |
| C42 | Knopf statt Automatik | „Ergebnisse eintragen → RaceClocker" auf einem Lauf ohne jede Zeit meldet jetzt **Erfolg** statt „keine Ergebnisse", und die Bahnen sind übernommen. Wie C29 ungewohnt zu lesen und trotzdem richtig | `aa3d209a` | 6fe40a37, 10.08. (CF): Wettkampf 1 Finale aktiviert, gegen ein importiertes WAVE-Testrennen (`raceclocker.com/0a77d582`, Feed nur „Not started") gepullt → **HTTP 204**, Bahnen nach `Rank` vergeben (Rang 1→Bahn 1 …), kein Fehler |
...
| C45 | Abgemeldetes Boot | Ein Boot ist abgemeldet und steht nicht mehr im Feed, der Lauf hat noch keine Zeiten: Es behält eine Nummer oberhalb der importierten Bahnen und kollidiert mit keiner echten Bahn (C13 für den ergebnislosen Fall) | `aa3d209a` | 6fe40a37, 10.08. (CF): das nicht im Feed stehende Boot (Verena Fetscher) landete nach dem Pull auf Bahn 6 — oberhalb der fünf importierten Bahnen, keine Kollision |
...
| ID | Fall | Erwartung | testbar ab | Nachweis |
|---|---|---|---|---|
| F16 | Rennen anlegen | Drei Rennen anlegen (Timetrials/Einzelstarts, Langstrecke/Läufe mit Rundenzeiten-Kennzeichen, Kurzstrecke/Läufe); alle drei erscheinen in der Liste mit Startart und Adresse | `a5d30cf3` | c6a37f63, 10.08. (CF): Rennen „Test Langstrecke", Läufe mit „Erfasst Rundenzeiten", angelegt und in der Liste mit Startart + Adresse |
| F17 | Voreinstellung anwählen | Quali → Timetrials, übrige Runden → Kurzstrecke; ein Wettkampf ohne eigene Anwahl zeigt im Zeitnahme-Tab genau diese beiden Namen als „geerbt" | `a5d30cf3` | c6a37f63, 10.08. (CF): Event auf Zeitfahren CF1x / Läufe CF1x (2) gesetzt; Wettkampf 7 zeigt im Tab „Vorgabe der Veranstaltung" genau diese beiden |
| F18 | Wettkampf weicht ab | Bei einem Wettkampf „Langstrecke" anwählen, speichern, Seite neu laden; im Event-Tab steht er unter „Wettkämpfe mit eigener Zeitnahme" **mit Rennennamen** | `a5d30cf3` | c6a37f63, 10.08. (CF): Wettkampf 7 auf „Test Langstrecke" für übrige Runden gesetzt; nach Reload im Event-Tab „eigenes Rennen: Test Langstrecke" |
| F19 | Anwahl wieder leeren | Override-Schalter aus → der Wettkampf erbt wieder, und die Abweichungs-Liste nennt ihn nicht mehr | `a5d30cf3` | c6a37f63, 10.08. (CF): Override-Schalter aus → competition.raceclocker_race_* auf null, Wettkampf erbt wieder |
...
| F21 | Rennen löschen | Ein angewähltes Rennen löschen: Rückfrage erscheint; danach erben die betroffenen Wettkämpfe wieder die Voreinstellung, und der Abruf überspringt Läufe ohne jedes Rennen still | `a5d30cf3` | c6a37f63, 10.08. (CF): „Test Langstrecke" gelöscht (Rückfrage kam), DELETE 204; competition.raceclocker_race_rounds des Wettkampfs 7 danach null |
| F22 | Doppelte Adresse abgewiesen | Zwei Rennen mit derselben Ergebnis-Adresse anlegen → verständliche Meldung statt „Unerwarteter Fehler"; dasselbe für zwei gleiche Namen | `a5d30cf3` | c6a37f63, 10.08. (CF): zweites Rennen auf `raceclocker.com/deadbeef` (Apex zu www-Bestand) → 409 RACECLOCKER_RACE_URL_TAKEN, verständliche Meldung |
...
| ID | Fall | Erwartung | testbar ab | Nachweis |
|---|---|---|---|---|
| N1 | Anstehende Läufe erscheinen überhaupt | Tab „Live" öffnen, ohne dass irgendein Lauf aktiviert ist: die nächsten Läufe stehen da, jeder mit Chip „Anstehend". Vorher war der Tab in dieser Lage **leer** — genau das war der Anlass | `7f7d7398` | eff2645a, 10.08. (CF): Live-Tab der Förde-Testregatta — alle anstehenden Läufe mit Chip „Anstehend“, auch die noch nicht erzeugten Runden |
...
| N3 | Dieselben Worte wie nebenan | Denselben Lauf gleichzeitig in Durchführung, Zeitplan, Dashboard und im Live-Tab ansehen: **gleicher Text, gleiche Farbe**. Das ist die eigentliche Zusage des Vorhabens — eine Abweichung hier ist ein Befund, auch wenn beide Seiten für sich plausibel aussehen | `7f7d7398` | eff2645a, 10.08. (CF): Vorlauf 2 nach Kettenaktivierung gleichzeitig „In Vorbereitung“ im Schiedsrichter-Dashboard, `state: PREPARING` auf dem Board und „In Vorbereitung“-Chip im Live-Tab |
...
| N11 | Wartende Runde und Programmpunkt | Eine noch nicht erzeugte Runde erscheint mit dem Hinweis „Aufstellung steht noch nicht fest" statt einer leeren Karte. Ein Programmpunkt („Mittagspause") erscheint **ohne** Chip — und wird auch eine halbe Stunde nach seiner Zeit **nicht** „Überfällig" genannt | `affc8ae5`, `248871b0` | eff2645a, 10.08. (CF): nicht erzeugte Finalrunden zeigen „Aufstellung steht noch nicht fest“ statt leerer Karte (Programmpunkt-Hälfte offen) |
